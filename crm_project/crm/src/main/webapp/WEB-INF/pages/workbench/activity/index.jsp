...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#editActivityBtn").click(function () {
			...
			// var id = chkedIds.val();  //拿到id的Value值
			// var id = chkedIds.get(0).value;  //转为dom 对象    方法都可以
		...
		$("#exportActivityXzBtn").click(function () {
			...
			window.location.href="workbench/activity/optionExportAllActivities.do?"+ids;
		...
		$("#importActivityBtn").click(function () {
			...
			//不但能向后台提交文本数据，还能提交二进制数据
			var formDate = new FormData();
			formDate.append("activityFile",activityFile);/* name  ,   value */

			//发送请求
			$.ajax({
				url:'workbench/activity/importActivity.do',
				data:formDate,
				processData:false,//设置Ajax向后台提交参数之前，是否把参数统一转换成字符串， 默认 true
				contentType:false,//设置Ajax向后台提交参数之前，是否把所有的参数统一按urlencoded编码，默认 true
				type:'post',
				dataType:'json',
				success:function (data) {
					if (data.code=="1"){
	...
	function queryActivityByConditionForPage(pageNo,pageSize) {
		...
		$.ajax({
			...
			success:function (data) {
				...
				//显示市场活动的列表，遍历activityList      index 下标；obj 循环变量，存值；
				var htmlStr="";
				$.each(data.activityList,function (index,obj) {
					htmlStr+="<tr class=\"active\">";
					htmlStr+="<td><input type=\"checkbox\" value=\""+obj.id+"\"/></td>";
					htmlStr+="<td><a style=\"text-decoration: none; cursor: pointer;\" onclick=\"window.location.href='workbench/activity/detailActivity.do?id="+obj.id+"'\">"+obj.name+"</a></td>";  //跳转到详情信息
