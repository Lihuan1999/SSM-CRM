...
<script type="text/javascript">
	...
	$(function(){
        ...
        /**
         * 根据clueId和activityName查询市场活动,  输入文本框，键盘弹出事件， 发送请求
         */
        $("#keyBoardText").keyup(function () {
            ...
            $.ajax({
                url:'workbench/clue/affiliatedMarketingActivities.do',
                type:'post',
                dataType:'json',
                data:{
        ...
        /**
         * 给关联按钮添加单击事件，进行市场活动和线索的 关联
         */
        $("#make_an_association").click(function () {

            //获取列表中所有被选中的checkbox
            var checkedlds = $("#tBody input[type='checkbox']:checked");

            if (checkedlds.size() == 0){
                alert("请选择要关联的市场活动!");
                return ;
            }

            //拼接参数 activityId+clueId
            var ids = "";
            $.each(checkedlds,function () {
				ids+="activityId="+this.value+"&";
            }); //activityId=xxx&activityId=xxx&clueId=xxx
            ids +="clueId=${clue.id}";

            $.ajax({
                url:'workbench/clue/saveGuanLianRelation.do',
                type:'post',
                dataType: 'json',
                data: ids,
                success:function (data) {
        ...
        $("#tBody2").on("click","a",function () {
            ...
            var clueId = '${clue.id}';
            alert(activityId+clueId);
            if(window.confirm("你确定解除关联吗?")){
                $.ajax({
		...
		$("#toConvertBtn").click(function () {
			...
			window.location.href="workbench/clue/convertClue.do?id="+id;
		});
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" id="keyBoardText" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="make_an_association">关联</button>
				</div>
			</div>
		</div>
	</div>
		...
		<c:forEach items="${remarkList}" var="remark">
			<div class="remarkDiv" id="div_${remark.id}" style="height: 60px;">
				...
				<div style="position: relative; top: -40px; left: 40px;" >
					...
					<font color="gray">线索</font> <font color="gray">-</font> <b>${clue.fullname}${clue.appellation}-${clue.company}</b> <small style="color: gray;"> ${remark.editFlag=='1'?remark.editTime:remark.createTime} 由${remark.editFlag=='1'?remark.editBy:remark.createBy}
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a href="javascript:void(0);" id="accActivityBtn" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
